...
note.prototype.update = function(show, state) {
    ...
    if(data.enhancedEditor && !tinyMCE.activeEditor) {
        this.createEnhancedEditor(true);
    }
    else if(!data.enhancedEditor && tinyMCE.activeEditor) {
        this.createEnhancedEditor(false);
    }
    this.updateText();
    this.updateAudio();
    this.resize();
...
note.prototype.settings = function() {
    if(arguments.length) {
        ...
        s += "<div class='ui-field-contain no-field-separator' data-role='controlgroup'>";
        s += printCheckbox("note_showConfirmation", "Show note save confirmation", data['showConfirmation']);
...
note.prototype.createEnhancedEditor = function(show) {
    if(show) {
        var mce_opts = {
            ...
            menubar: false,
            plugins: [
                'advlist charmap colorpicker fullscreen hr link',
                'lists spellchecker wordcount'
            ],
            toolbar: 'fullscreen | undo redo | insert | styleselect | bullist numlist',
