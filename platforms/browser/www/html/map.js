...
map.prototype.createMap = function() {
    ...
    this.addMarker(0, latlng);
    this.addMarkers(data);
    this.addLine(0, [latlng]);
    this.addLines(data);
    this.createButtons();
};
...
map.prototype.addMarker = function(index, e) {
    ...
    if(index==0) {
        ...
        latlng = {"lat": e.data.location[1], "lng": e.data.location[2]};
    ...
    function cb(val) {
        if(val) {
            ...
            if(val==2) { // delete the event
                ...
                event[E_DATA].location[1] = parseFloat(pos[0]);
                event[E_DATA].location[2] = parseFloat(pos[1]);
    ...
    for (var i=0; i<events.length; i++) {
        ...
        if(e &&
           ...
           typeof(e.data.location)!="undefined" ) {
            this.addMarker(markerIndex++, e);
        }
    }
    // remove old markers
    for(var i=markerIndex; i<this.marker.length; i++) {
        this.map.removeLayer(this.marker[i]);
        this.marker.splice(i,1);
    }
};
map.prototype.addLine = function(index, p) {
...
map.prototype.addLines = function(data) {
    ...
    // remove old lines
    for(lineIndex=this.line.length-1; i>=1; i--) {
        this.map.removeLayer(this.line[i]);
