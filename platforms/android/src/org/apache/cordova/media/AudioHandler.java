...
public class AudioHandler extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equals("startRecordingAudio")) {
           ...
           try {
            ...
            callbackContext.sendPluginResult(new PluginResult(status, f));
            return true;
        } else if (action.equals("getRecordingPower")) {
            float f = this.getRecordingPower(args.getString(0));
            callbackContext.sendPluginResult(new PluginResult(status, f));
            return true;
        }
    ...
    /**
     * Get current recording power
     * @param idThe id of the audio player
     * @return The maximum signal power since the last call
     */
    public float getRecordingPower(String id) {
        AudioPlayer audio = this.players.get(id);
        if (audio != null) {
            float a = audio.getMaxRecordingAmp();
            return a*a;
        }
        return 0;
    }
