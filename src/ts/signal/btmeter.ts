...
//import {Mooshimeter} from '../3p/mooshimeter';

import * as $ from 'jquery';

//import {PGMeter,PGMooshimeter,GenericMeter} from './btmeter'

export class Bluetoothometer extends Meter {
    device = null;
    ...
    stopNotification(service, characteristic) {
        pgUtil.ble.stopNotification(this.device.id, service, characteristic, () => {
        }, () => {
        });
    }
    
    readBT(service, characteristic, readCB, writeCB) {
        if (!this.device) {
            pgDebug.showError('Tried to read from non-existent device');
            return;
        }
        pgUtil.ble.read(this.device.id, service, characteristic, readCB, writeCB);
    }
    writeBT(service, characteristic, bytes, readCB, writeCB) {
        if (!this.device) {
            pgDebug.showError('Tried to read from non-existent device');
            return;
        }
        pgUtil.ble.write(this.device.id, service, characteristic, bytes, readCB, writeCB);
    }
}

//====================================================

export class GenericMeter extends Bluetoothometer {
    period = 1000;
    device = null;
    deviceHandle = 0;
    deviceService = '';
    ...
    connect(callback) {
        ...
        const characteristics = this.device.characteristics;
        for (let i = 0; i < characteristics.length; i++) {
            ...
            if (characteristics[i].service === this.deviceService) {
                const hasNotify = characteristics[i].properties.indexOf('Notify') !== -1;
                const hasRead = characteristics[i].properties.indexOf('Read') !== -1;
    ...
    pickServiceCharacteristic(callback = (service, characteristic) => {}) {
        ...
        function bleServiceCallback(sID) {
            ...
            const service = this.device.services[sID];
            pgDebug.showLog('services returned with: ' + service);
            const characteristics = this.device.characteristics;
            const cIDs = [];
            for (let i = 0; i < characteristics.length; i++) {
                // only add characteristics that can be read, or that have the notify property
            ...
            function addServiceCB(cID) {
                ...
                const characteristic = cIDs[cID];
                pgDebug.showLog('Adding service ' + service + '; characteristic:' + characteristic);
    ...
    start(restart) {
        if (this.hasNotify()) {
            this.startNotification(this.deviceService, this.deviceCharIn, this.read.bind(this));
            this.deviceHandle = 0;
        } else {
            this.deviceHandle = setInterval(readCallback.bind(this), this.period);// eslint-disable-line
        }
        if (!restart) {
        ...
        function readCallback() {
            this.readBT(this.deviceService, this.deviceCharIn, this.read.bind(this), this.readError);
        }
    }
    async stop() {
        this.running = false;
    ...
    read(obj) {
        ...
        if (bytes.length) {
            if (this.signals[0] === 'heartRate') {
                ...
                const val = [time, num];
                pgDebug.showLog('Read data: ' + val);
                this.pushData(val);
            } else {
                const time = pgUtil.getCurrentTime();
                if (bytes.length === 4) {
                    const num = this.decodeUint32(bytes);
                    const val = [time, num];
                    this.pushData(val);
                } else if (bytes.length === 2) {
                    const num = this.decodeUint16(bytes);
                    const val = [time, num];
                    this.pushData(val);
                } else {
    ...
    servicePicker(services, callback) {
    ...
    characteristicPicker(characteristics, callback) {
...
export class PGMeter extends Bluetoothometer {
    pgService = '00000000-ABBA-DABA-B000-197219721972';
    pgCharSettings = '00000100-ABBA-DABA-B000-197219721972';
    pgCharData = '00000200-ABBA-DABA-B000-197219721972';
    pgCharButton = '00000300-ABBA-DABA-B000-197219721972';
    pgCharMotion = '00000310-ABBA-DABA-B000-197219721972';
    pgCharTemp = '00000400-ABBA-DABA-B000-197219721972';
    pgCharAccel = '00000410-ABBA-DABA-B000-197219721972';
    pgCharAnalog = '00000420-ABBA-DABA-B000-197219721972';
    
    firstTime = 0;
    mode = 0;
    period = 1000;
    alarms = ['sound'];
    allAlarms = ['vibrate', 'sound', 'lights'];
    motion = 0;
    button = 1;
    allPeriods = [20, 50, 100, 1000, 10000];
    signals = ['temperature'];
    allSignals = ['temperature', 'acceleration', 'analog1', 'analog2'];
    ...
    settingsDialog(callback) {
        const opts = this.settingsDialogOpts("PG Meter", gatherData);
        let optionText = '';
        ...
        function gatherData() {
            return {
                ...
                period: ()=>{parseInt($('#meter_period').val())},
                alarms: ()=>{$('#meter_alarm').val() || []},
            };
        }
        function setMeter(success, data) {
            if (success) {
    ...
    connect(callback) {
        this.setParameters();
        this.startNotification(this.pgService, this.pgCharButton, this.read.bind(this, 'button'));
        this.startNotification(this.pgService, this.pgCharMotion, this.read.bind(this, 'motion'));
        this.startNotification(this.pgService, this.pgCharTemp, this.read.bind(this, 'temperature'));
        this.startNotification(this.pgService, this.pgCharAccel, this.read.bind(this, 'acceleration'));
        this.startNotification(this.pgService, this.pgCharAnalog, this.read.bind(this, 'analog'));
        callback(true);
    }
    disconnect(callback) {
    ...
    read(type, a) {
        ...
        const ts = this.decodeUint32(bytes);
        if (!this.firstTime) {
            const time = pgUtil.getCurrentTime();
        ...
        if (type === 'button') {
            ...
            const val = this.decodeUint16(bytes.slice(4));
            const pt = [this.firstTime + ts, val];
            if (val === 1) {
                pgUI[pgUI.page()].lever('left');
            } else if (val === 2) {
                pgUI[pgUI.page()].lever('right');
            }
        } else if (type === 'motion') {
            // 4 bytes timestamp, 2 bytes step
            const val = this.decodeUint16(bytes.slice(4));
        ...
        if (this.signals.indexOf(type) === -1 && (type !== 'analog')) {
            pgDebug.showWarn('Not logging signal: ' + type);
            return;
        }
        if (type === 'temperature') {
            // 4 bytes timestamp, 2 bytes temp
            let val = this.decodeSingle(bytes.slice(4));
            val = this.filter.push(val);
            const pt = [this.firstTime + ts, val];
            this.addData(type, pt);
        } else if (type === 'acceleration') {
            // 4 bytes timestamp, 6 bytes temp
            const ax = this.decodeSingle(bytes.slice(4));
            const ay = this.decodeSingle(bytes.slice(8));
            const az = this.decodeSingle(bytes.slice(12));
            const pt = [this.firstTime + ts, ax, ay, az];
            this.addData(type, pt);
        } else if (type === 'analog') {
            // 4 bytes timestamp, 2 bytes temp
            let val = 0;
    ...
    write(type) {
        ...
        if (type === 'mode') {
            ...
            sel = 6;
            if (this.signals.indexOf('temperature') >= 0) {
                val = 1;
            }
        } else if (type === 'acceleration') {
            sel = 7;
            if (this.signals.indexOf('acceleration') >= 0) {
                val = 1;
            }
        } else if (type === 'analog') {
            sel = 8;
        ...
        this.writeBT(this.pgService, this.pgCharSettings, bytes.buffer, this.writeSuccess, this.writeError);
    }
}

// =================================================================================
/*
export class PGMooshimeter extends Bluetoothometer {
    deviceService  = "1BC5FFA0-0200-62AB-E411-F254E005DBD4";
    deviceCharOut  = "1BC5FFA1-0200-62AB-E411-F254E005DBD4";
    deviceCharIn   = "1BC5FFA2-0200-62AB-E411-F254E005DBD4";
    ...
    signals        = ["resistance"];
    allSignals     = ["current","voltage","temperature","resistance"];
    SR             = 125;
    allSR          = [125, 250, 500, 1000, 2000, 4000, 8000];
    SD             = 32;
    allSD          = [32, 64, 128, 256];
    settingA       = 10;
    settingV       = 0.1;
    ...
    rangeA         = [1, 2.5, 10];
    rangeV         = [1.2, 60, 600];
    rangeAuxV      = [0.1, 0.3, 1.2];
    rangeAllV      = [0.1, 0.3, 1.2, 60, 600];
    rangeR         = [1000, 10000, 100000, 1000000, 10000000];
    ...
    update(show, data) {
        try {
            if(show) {
                this.signals      = data.signals;
                this.settingA     = data.settingA;
                this.settingV     = data.settingV;
                this.settingR     = data.settingR;
                this.SR           = data.SR;
                this.SD           = data.SD;
            }
            else {
                data.signals      = this.signals;
                data.settingA     = this.settingA;
                data.settingV     = this.settingV;
                data.settingR     = this.settingR;
                data.SR           = this.SR;
                data.SD           = this.SD;
            }
        }
    ...
    settingsDialog(callback) {
        const title = "Mooshimeter options";
        let optionText = "";
        ...
        super.settingsDialog(title, optionText, setMeter.bind(this));// eslint-disable-line
    ...
    initializeMeter() {
        const srIndex = this.allSR.indexOf(this.SR);
        this.meter.sendCommand('sampling:rate ' +srIndex);
        const sdIndex = this.allSD.indexOf(this.SD);
        this.meter.sendCommand('sampling:depth ' +sdIndex);

        const ch1 = ["Current","Temperature","Shared"];
        const ch2 = ["Voltage","Temperature","Shared"];
        this.channel = 1;
        ...
        if(this.signals[0]==="current") {
            ...
            const index = 1+ this.rangeA.indexOf(this.settingA);
            ...
            if(this.settingV <= 1.2) { // shared V
                ...
                const index = 1 + this.rangeAuxV.indexOf(this.settingV);
                ...
                this.meter.sendCommand('shared 0');          // shared: voltage
                const index = 1 + this.rangeV.indexOf(this.settingV);
            ...
            const index = 1+ this.rangeR.indexOf(this.settingR);
            ...
            pgDebug.showError("unknown meter type");
        }
    ...
    connect(callback) {
        ...
        setTimeout(mconnect.bind(this), 1000);// eslint-disable-line
        function mconnect() {
            this.meter.connect(run.bind(this));// eslint-disable-line
        }
        function run(success) {
            this.initializeMeter();
            this.ping = setInterval(periodic.bind(this), 4000);// eslint-disable-line
            callback(success);
            function periodic() {
    ...
    meterCB(signal, m, val) {
        val = this.filter.push(val);
        const tval = [pgUtil.getCurrentTime(), val];
        this.addData(signal, tval);
    }
    stop(callback) {
        this.running = false;
        this.meter.attachCallback('ch1:value', NOP.bind(this));// eslint-disable-line
        this.meter.attachCallback('ch2:value', NOP.bind(this));// eslint-disable-line
