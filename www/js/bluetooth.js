...
var pgBluetooth = {
    ...
    getActiveDeviceName: function() {
        return pgBluetooth.activeDeviceName;
    },
    setActiveDeviceName: function(name, cb) {
        if(!name)
            name = "none";
        if(pgBluetooth.activeDeviceName != name) {
            pgBluetooth.disconnect(cbc);
        }
        else
            cbc();
        function cbc() {
            if(name != "none") {
                pgBluetooth.connect(name, cb);
            }
            else
                cb(false);
        }
    ...
    disconnect: function(cb) { 
        ...
        pgBluetooth.activeDeviceName = "none";
        if(pgBluetooth.useBLE) {
    ...
    isConnected: function(cb) {
        ...
        function yes() {cb(true);}
        function no()  {cb(false);}
    },
    ...
    connect: function(name, cb) {
           ...
           pgUtil.isWebBrowser()
        ...
        else if(! $.isEmptyObject(pgBluetooth.activeDevice)){
            pgBluetooth.isConnected(connectCB);
        }
        else {
            ...
            for(var i=0; i<btDevs.length; i++) {
                if( btDevs[i].name == name) {
                    //btDevs[i].address != pgBluetooth.activeDevice.address)
                    if(pgBluetooth.useBLE)
                        address = btDevs[i].id;
                    else
        ...
        function connectSuccess(result) {
            if(pgBluetooth.useBLE) {
                pgBluetooth.activeDeviceName = result.name;
            ...
            if (result.status === "connected") {
                pgBluetooth.activeDeviceName = name;
                pgBluetooth.activeDevice = { address: result.address };
    ...
    start: function(opts) {
        ...
        pgBluetooth.connect(pgBluetooth.activeDeviceName, cb);
        function cb() {
            pgBluetooth.data = [];
        ...
        function readSuccess(obj) {
            ...
            //showLog("Bluetooth Read Success : " + JSON.stringify(obj));    
            if (bytes.length) {
    ...
    handleError: function(error) {
        ...
        pgBluetooth.isConnected(cb);
        function cb(connected) {
            if(connected)
                pgBluetooth.disconnect(function(){});
            showError(msg);
        }
    },
    ...
    servicePicker: function(services, cb) {
        ...
        showDialog({'title': pgBluetooth.getActiveDeviceName(), true: "OK"},
                   dialog_content,
                   dialog_cb, "prefs");
        function dialog_cb() {
